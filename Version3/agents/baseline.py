...
class ReplayBuffer:
    def __init__(self, capacity=1000000):
        self.buffer = deque(maxlen=capacity)
        self.experience = namedtuple("Experience", field_names=["obs", "acts", "rews", "next_obs", "dones"])
        self.max_dim = {'obs': 0, 'acts': 0, 'rews': 0}  # 跟踪最大维度

    def add(self, obs, acts, rews, next_obs, dones):
        """添加经验到缓冲区"""
        obs_list = self._dict_to_list(obs)
        next_obs_list = self._dict_to_list(next_obs)
        acts_list = self._dict_to_list(acts)
        rews_list = self._dict_to_list(rews)
        dones_list = self._dict_to_list(dones)

        # 更新最大维度
        self.max_dim['obs'] = max(self.max_dim['obs'], len(obs_list))
        self.max_dim['acts'] = max(self.max_dim['acts'], len(acts_list))
        self.max_dim['rews'] = max(self.max_dim['rews'], len(rews_list))

        # 填充到固定维度
        obs_list = self._pad_list(obs_list, self.max_dim['obs'])
        next_obs_list = self._pad_list(next_obs_list, self.max_dim['obs'])
        acts_list = self._pad_list(acts_list, self.max_dim['acts'])
        rews_list = self._pad_list(rews_list, self.max_dim['rews'])
        dones_list = self._pad_list(dones_list, self.max_dim['rews'])  # 使用rews的维度

        e = self.experience(obs_list, acts_list, rews_list, next_obs_list, dones_list)
        self.buffer.append(e)

    def _pad_list(self, data_list, target_length):
        """将列表填充到目标长度"""
        if len(data_list) < target_length:
            return data_list + [0.0] * (target_length - len(data_list))
    ...
    def sample(self, batch_size):
        ...
        if not experiences:
            raise ValueError("No valid experiences found in sample")
