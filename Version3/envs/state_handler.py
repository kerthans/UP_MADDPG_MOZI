...
class StateHandler:
    ...
    def reset(self):
        ...
        return self.get_obs()  # 不再需要传入thread_pool
    ...
    def enhance_observation(self, state_vector, unit_id, is_red, all_obs):
        """增强单个智能体的观察向量

        Args:
            state_vector (np.ndarray): 基础状态向量
            unit_id (str): 单位ID
            is_red (bool): 是否为红方
            all_obs (dict): 所有单位的观察

        Returns:
            np.ndarray: 增强后的观察向量
        """
        # 解析单位索引
        idx = int(unit_id.split('_')[1]) if unit_id.split('_')[1].isdigit() else 0

        # 确保状态矩阵已初始化
        if self.red_states is None:
            self.red_states = np.zeros((self.config['num_red'], 5))
        if self.blue_states is None:
            self.blue_states = np.zeros((self.config['num_blue'], 5))
        ...
        own_state[:2] = state_vector[:2] * self.config['field_size']  # 位置
        own_state[2] = np.arctan2(state_vector[5], state_vector[4])  # 航向
        own_state[3] = state_vector[3] * self.config['max_speed']  # 速度
        own_state[4] = 1.0  # 存活状态

        # 使用现有的_get_agent_obs方法获取完整观察
        return self._get_agent_obs(idx, is_red)
    ...
    def _get_agent_obs(self, idx: int, is_red: bool) -> np.ndarray:
        ...
        if team_states is not None:  # 安全检查
            for i, state in enumerate(team_states):
                if i != idx and teammate_count < max_teammates:
                    rel_pos = state[:2] - own_pos
                    dist = np.linalg.norm(rel_pos)
        ...
        if opponent_states is not None:  # 安全检查
            for state in opponent_states:
                if opponent_count < max_opponents:
                    rel_pos = state[:2] - own_pos
                    dist = np.linalg.norm(rel_pos)
